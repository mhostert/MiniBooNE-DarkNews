...
def fit_point_3p2(
    ...
    **kwargs,
    ...
    df = GenLauncher(m5=m5s, m4=m4s, experiment=experiment, **kwargs).run()
    decay_l = df.attrs["N5_ctau0"]

    # Run the generation for MiniBooNE Dirt
    df2 = GenLauncher(m5=m5s, m4=m4s, experiment=experiment + "_dirt", **kwargs).run()
    ...
    if "miniboone" in experiment:
        ...
        df_tot = analysis.reco_nueCCQElike_Enu(df_tot, cut=cut)
        df_tot = df_tot[df_tot.reco_w > 0]
        sum_w_post_smearing = np.abs(np.sum(df_tot["reco_w"]))
        ...
        if print_spectra:
            ...
            # Evis
            line = " ".join(
                [f"{i:.6e}" for i in ff.get_MiniBooNE_Evis_bin_vals(df_tot)]
            )
            gf.save_to_locked_file(
