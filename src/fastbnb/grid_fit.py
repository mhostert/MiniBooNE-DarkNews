...
def get_data_MB(varplot="reco_Evis"):
    if varplot == "reco_Evis":
        _, data = np.genfromtxt(resources.open_text("fastbnb.include.miniboone_2020", "Evis_data.dat"), unpack=True)
        _, bkg = np.genfromtxt(resources.open_text("fastbnb.include.miniboone_2020", "Evis_bkg.dat"), unpack=True)
        signal = data - bkg
        sys_signal = 0.1
        sys_bkg = 0.1
        ...
        _, data = np.genfromtxt(resources.open_text("fastbnb.include.miniboone_2020", "Enu_data.dat"), unpack=True)
        _, bkg = np.genfromtxt(resources.open_text("fastbnb.include.miniboone_2020", "Enu_constrained_bkg.dat"), unpack=True)
        _, error_low = np.genfromtxt(resources.open_text("fastbnb.include.miniboone_2020", "Enu_lower_error_bar_constrained_bkg.dat"), unpack=True)
        signal = data - bkg
        sys_bkg = (bkg - error_low) / bkg
        sys_signal = 0.1
        bin_e = np.genfromtxt(resources.open_text("fastbnb.include.miniboone_2020", "Enu_bin_edges.dat"), unpack=True)
        bin_w = bin_e[1:] - bin_e[:-1]
        signal *= bin_w * 1e3
        bkg *= bin_w * 1e3

    elif varplot == "reco_angle":
        _, data = np.genfromtxt(resources.open_text("fastbnb.include.miniboone_2020", "cosTheta_data.dat"), unpack=True)
        _, bkg = np.genfromtxt(resources.open_text("fastbnb.include.miniboone_2020", "cosTheta_bkg.dat"), unpack=True)
        signal = data - bkg
        sys_signal = 0.1
        sys_bkg = 0.1
    ...
    def __init__(
        ...
        if model == "3+1":
            ...
            self.vmu4_def = (
                ...
                * self.couplings_def["Umu4"]
                / np.sqrt(1 - self.couplings_def["Umu4"] ** 2)
            )
        elif model == "3+2":
            self.couplings_def = {
                "UD4": UD4_def,
                ...
                "Umu4": Umu4_def,
                "Umu5": Umu5_def,
                "epsilon": epsilon_def if epsilon_def else 1e-2,
            }
            self.vmu5_def = (
                self.couplings_def["gD"]
        ...
        if model == "3+1":
            self.couplings = {
                ...
                "gD": gD if gD else gD_def,
                "Umu4": Umu4 if Umu4 else Umu4_def,
            ...
            self.couplings = {
                "UD4": UD4 if UD4 else UD4_def,
                "UD5": UD5 if UD5 else UD5_def,
                "gD": gD if gD else gD_def,
                "Umu4": Umu4 if Umu4 else Umu4_def,
                "Umu5": Umu5 if Umu5 else Umu5_def,
                "epsilon": epsilon if epsilon else 1e-2,
            }
            self.vmu5 = (
                lambda umu: self.couplings["gD"]
                * self.couplings["UD5"]
                * (umu[0] * self.couplings["UD4"] + umu[1] * self.couplings["UD5"])
                / np.sqrt(1 - umu[0] ** 2 - umu[1] ** 2)
            )
        ...
        if self.model == "3+1":
            ...
            def chi2_grid_run(k_y):
                for k_x in range(self.grid_params["x_points"]):
                    ...
                    try:
                        pd.read_parquet(
                            location + "/" + self.experiment + f"/3plus1/m4_{m4s}_mzprime_{mzs}_" + self.HNLtype + "/pandas_df.parquet", engine="pyarrow"
                        )
                        ...
                        try:
                            gen = GenLauncher(
                                mzprime=mzs,
                                m4=m4s,
                                Umu4=self.couplings_def["Umu4"],
                                UD4=self.couplings_def["UD4"],
                                gD=self.couplings_def["gD"],
                                epsilon=self.couplings_def["epsilon"],
                                neval=self.neval,
                    ...
                    try:
                        ...
                        try:
                            gen = GenLauncher(
                                ...
                                m4=m4s,
                                Umu4=self.couplings_def["Umu4"],
                                Umu5=self.couplings_def["Umu5"],
                                UD4=self.couplings_def["UD4"],
                                UD5=self.couplings_def["UD5"],
                                gD=self.couplings_def["gD"],
                                epsilon=self.couplings_def["epsilon"],
                                neval=self.neval,
                                HNLtype=self.HNLtype,
            ...
            def chi2_grid(k_y):
                ...
                try:
                    ...
                    for k_x in range(self.grid_params["x_points"]):
                        ...
                        chi2_enu = lambda theta: ff.chi2_MiniBooNE_2020_3p1(
                            df, A_cut * np.cos(theta) ** 2, on_shell=on_shell, r_eps=self.r_eps, l_decay_proper_cm=decay_l
                        )
                        ...
                        if on_shell:
                            ...
                            df_decay = av.select_MB_decay_expo_prob(
                                df, coupling_factor=self.v4i(umu4_bf) / self.v4i_def * self.r_eps, l_decay_proper_cm=decay_l
                            )
...
class grid_analysis_couplings:
    def __init__(
        ...
        x_label="mzprime",
        x_range=(0.02, 10, 10),
        coupling_range=(1e-4, 1e-2, 10),
        log_interval_x=True,
        log_interval_coupling=True,
        mzprime=None,
        m4=None,
        m5=None,
        delta=None,
    ...
    def generate_events(self, location="data", **kwargs):
        if self.model == "3+1":
            ...
            def chi2_grid_run(k_x):
                ...
                try:
                    pd.read_parquet(
                        location + "/" + self.experiment + f"/3plus2/m5_{m5s}_m4_{m4s}_mzprime_{mzs}_" + self.HNLtype + "/pandas_df.parquet", engine="pyarrow"
                    )
                except:
                    try:
                        gen = GenLauncher(
