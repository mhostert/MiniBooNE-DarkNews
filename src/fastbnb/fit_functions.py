...
class exp_plotter:
    def __init__(
        ...
        points=20,
        model="3+2",
        delta=np.geomspace(0.05, 79, 20),
        m4=np.geomspace(0.01, 0.75, 20),
        mzprime=1.25,
        m5_max=10.0,
        neutrino_type="majorana",
        exp="miniboone",
        nb_cores=8,
        mode_chi2=True,
        sys_angle=0,
        ...
        self.paths = {
            ...
            "couplings": self.paths_coup,
            "original": self.paths_original,
            "original_couplings": self.paths_coup_original,
        }
    ...
    def plot_couplings_mass(
        self,
        type_fit="Enu",
        leg_loc="upper left",
        coupling_exp=0,
        save=True,
        min_enu=["function", "function"],
        min_angle=["function", "function"],
        chi2_min=None,
        xlims=["none", "none"],
    ):
                ...
                if self.model == "3+1":
                    ...
                    if self.coupling_mode == "m4":
                        ...
                        plot_title2 = (
                            ...
                            + str(self.mzprime)
                            + r"$ \ \mathrm{GeV}$,$\Delta = $"
                            + str(round_sig([data.loc[0, "delta"]]))
                            + ",MiniBooNE, "
                            + self.hnl_type
                        )
    ...
    def plot_bf_xsection(self, definition_number=500, fit_source="Enu", regime="coherent", gD=1.0, epsilon=1e-2, Umu4=1e-6, UD4=1.0):
        ...
        fig.savefig(
            self.path
            + "__grid_run__/plots/xsec_3+1_"
            + fit_source
            + f"_{mc.ups_case.mzprime:.2f}_{mc.ups_case.m_ups*1e3:.0f}_{regime}_"
            + self.neutrino_type
            + "_"
            + self.method
            + ".pdf",
    ...
    def do_whole_analysis(
        ...
        couplings_light_angle=np.geomspace(0.1e-23, 2.5e-23, 40),
        couplings_heavy_enu=np.geomspace(0.1e-23, 2.5e-23, 40),
        couplings_heavy_angle=np.geomspace(0.1e-23, 2.5e-23, 40),
        couplings_mode="m5",
        im_light=2,
        im_heavy=18,
        coupling_exp_light=13,
        coupling_exp_heavy=13,
        rounding=2,
    ):
        if processes[0]:
            print("Running grid")
    ...
    def do_couplings_analysis(
        ...
        m4=np.geomspace(0.01, 0.75, 20),
        couplings_mode="m5",
        couplings_enu=np.geomspace(1e-5, 1e-1, 50),
        couplings_angle=np.geomspace(1e-5, 1e-1, 50),
        fit_enu=True,
        fit_angle=True,
    ):
