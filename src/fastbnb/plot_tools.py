...
def batch_plot_signalMB(
    obs, PATH, title="Dark News", Nevents=None, mode="fhc", loc="", prefix="", bkg_subtract=False, annotation="", xy_annotation=(0.65, 0.55)
):
    if Nevents is not None:
        total_Nevent_MB = Nevents
    else:
        total_Nevent_MB = obs["reco_w"].sum()
    print(total_Nevent_MB)
    #################### HISTOGRAMS 1D - STACKED ####################################################
    histogram1D_data_stacked(
        Path(PATH) / f"{prefix}_1D_Enu_data_stacked.pdf",
        obs,
        r"$E_\nu^{\rm CCQE-reco}/$GeV",
        title,
        varplot="reco_Enu",
        ...
        xy_annotation=xy_annotation,
    )
    histogram1D_data_stacked(
        Path(PATH) / f"{prefix}_1D_Evis_data_stacked.pdf",
        obs,
        r"$E_{\rm vis}^{\rm reco}/$GeV",
        title,
        varplot="reco_Evis",
        tot_events=total_Nevent_MB,
        loc=loc,
    ...
    histogram1D_data_stacked(
        Path(PATH) / f"{prefix}_1D_costheta_data_stacked.pdf",
        obs,
        r"$\cos\theta^{\rm reco}$",
        title,
...
def histogram1D_data_stacked(
    ...
    loc="../",
    mode="fhc",
    include_helicity=False,
    bkg_subtract=False,
    annotation="",
    xy_annotation=(0.75, 0.45),
):
    # Masks
    ...
    if include_helicity:
        ...
        cases = [coherent, pel, nel]
        case_shorthands = [r"coherent", r"proton-elastic", r"neutron-elastic"]

    colors = ["steelblue", "orange", "purple", "pink"]
    hatches = ["/////////////////////", "//////////////////", "\\\\\\\\\\\\\\\\\\\\\\\\\\", ""]
    ...
    fsize = 11
    fig, ax = pt.std_fig(figsize=(1.4 * 3.7, 2.6), ax_form=[0.16, 0.19, 0.81, 0.73])

    #####################
    # MiniBooNE data
    if varplot == "reco_Evis":
        if mode == "rhc":
            print("RHC mode not supported for Evis")
            return 0
        # miniboone nu data
        ...
        data_plot(
            ...
            bin_c,
            bin_w,
            (data_MB_enu_nue - (bkg_subtract) * data_MB_bkg),
        ...
        data_MB = np.genfromtxt(
            resources.open_text(
                f"fastbnb.include.MB_data_release.{mode}mode",
                f"miniboone_nue{'bar' if mode == 'rhc' else ''}data_lowe.txt",
            ),
            unpack=True,
        )

        data_MB_bkg = np.genfromtxt(
            resources.open_text(
                f"fastbnb.include.MB_data_release.{mode}mode",
                f"miniboone_nue{'bar' if mode == 'rhc' else ''}bgr_lowe.txt",
            ),
            unpack=True,
        )
        ...
        data_MB_bkg = data_MB_bkg / units
        data_MB_enu_nue = data_MB / units
        error_bar = np.sqrt(data_MB) / units

        data_plot(
            ax,
            bin_c,
            bin_w,
            data_MB_enu_nue - data_MB_bkg * bkg_subtract,
            error_bar / 2,
            error_bar / 2,
        )
        ...
        if mode == "rhc":
            print("RHC mode not supported for cosTheta_beam")
            return 0
        # miniboone nu data
        ...
        data_plot(
            ...
            bin_c,
            bin_w,
            (data_MB_cost_nue - bkg_subtract * data_MB_bkg),
    ...
    for i in range(len(cases)):
        ...
        _, _, bars = ax.hist(
            bins[:-1],
            weights=h + htotal + data_MB_bkg * (not bkg_subtract),
            bins=bin_e,
            ec="black",
            fc=colors[i] if hatches[i] == "" else "white",
            alpha=1,
            histtype="stepfilled",
            lw=0.0 if hatches[i] == "" else 0.75,
            hatch=hatches[i],
            label=legends[i],
            zorder=len(cases) - i - 1,
        )
        for patch in bars:
            patch._hatch_color = mpl_colors.to_rgba(colors[i])

        hists.append(h)
        htotal += h
    ax.step(
        np.append(bins[:-1], 10e10),
        np.append(htotal + data_MB_bkg * (not bkg_subtract), 0.0),
        where="post",
        c="black",
        lw=0.5,
    )
    ...
    ax.annotate(annotation, xy=xy_annotation, xycoords="axes fraction", ha="left", va="center", fontsize=fsize)
    ax.legend(frameon=False, loc="best")
    ax.set_xlabel(XLABEL, fontsize=fsize)
    ax.set_xlim(np.min(bin_e), np.max(bin_e))

    if varplot == "reco_Enu":
        ax.set_ylim(-0.1 * (bkg_subtract), ax.get_ylim()[1] * 1.1)
        ax.set_ylabel(r"Excess Events/MeV", fontsize=fsize)
    else:
        ax.set_ylim(-10 * (bkg_subtract), ax.get_ylim()[1] * 1.1)
        ax.set_ylabel(r"Excess events" if bkg_subtract else "Events", fontsize=fsize)

    pt.std_savefig(fig, plotname, dpi=400, bbox_inches="tight")


def errorband_plot(ax, X, BINW, DATA, ERRORLOW, ERRORUP, band=False, **kwargs):
    _ = ax.hist(X, bins=X + BINW / 2, weights=DATA, histtype="step")
    _ = ax.bar(X, 2 * ERRORUP, width=BINW, bottom=DATA - ERRORLOW, alpha=0.3, **kwargs)
...
def data_plot(ax, X, BINW, DATA, ERRORLOW, ERRORUP, band=False, **kwargs):
    ax.errorbar(
        X,
        DATA,
        yerr=np.array([ERRORLOW, ERRORUP]),
        xerr=BINW / 2.0,
        marker="o",
        markeredgewidth=0.5,
        capsize=0.5,
...
def histogram1D(
    ...
    fsize = 10
    fig = plt.figure()
    ax = fig.add_axes(pt.std_axes_form)
